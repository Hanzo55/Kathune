<cfcomponent displayname="Tentacle" output="false">
    ...
    <cfscript>
        ...
        variables.typo = 'rouge|drood|furry|worrier|warruir|shamy|serius|shamman|giuld|desto|walrlock|huner|paly|palidan|atunned|dorf|experinced|rading|holydin|loladin|hardocre|grull|preist|deth';
    ...
    <cffunction name="init" returntype="hanzo.cf.Kathune.KathuneTentacle" access="private" output="false">
        ...
        5. parse the HTML for the forum url, and return the parsed data as a query ( ::Grab() )
        6. todo: parse the HTML for the thread detail url, and return the parsed data as a string (Parse:fetchPost)
        7. todo: know how to determine if a post is valid (may no longer exist, may timeout, may be a 404, may be removed, etc) (ITentacle:isPostValid)
        8. todo: provide a region

        base functionality across all tentacles should be:

        9. todo: know how to pull the server type from a title (Parse:getServerTypeFromTitle)
        10. take the title of a post, and and convert it to a postStruct.
        11. strip XML-style tags (Parse:stripTags)
        12. parse an Armory URL out of an html post
        --->
    ...
    <cffunction name="CreatePostObjectFromQueryRow" returntype="hanzo.cf.Kathune.Post" access="public" output="false">
        ...
        <cfset postObject = CreateObject( 'component', 'hanzo.cf.Kathune.Post' ) />
    ...
    <cffunction name="getServerTypeFromTitleByRegion" returntype="struct" output="false" access="public">
        ...
        <cfloop query="qryServers">
            <cfif FindNoCase( qryServers.ServerName[qryServers.currentRow], arguments.txt ) OR
                    ( Len( qryServers.ServerRegExp[qryServers.currentRow] ) AND
                    ReFindNoCase( qryServers.ServerRegExp[qryServers.currentRow], arguments.txt ) )>
    ...
    <cffunction name="canBanText" returntype="boolean" access="public" output="false">
        ...
        <cfreturn Iif( ( ReFindNoCase( variables.bannedPhrases, arguments.textString ) OR
                Find( '?', arguments.textString ) ), true, false ) />
    </cffunction>
        ...
        <cfscript>
            ...
            if ( lfPos AND guildPos )
            {
                ...
                else
                {
                    ...
                    // changed: we now exclude any string of text with a ? mark, so this additional cpu cycle is unnecessary
